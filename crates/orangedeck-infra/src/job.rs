...
mod tests {
    ...
    #[test]
    fn every_job_kind_maps_to_a_fixed_cargo_invocation() {
        let arguments: Vec<&[&str]> = JobKind::ALL
            .iter()
            .copied()
            .map(JobKind::cargo_args)
            .collect();
        assert_eq!(
            arguments,
            vec![
    ...
    #[test]
    fn cargo_output_lines_are_bounded() {
        let line = "x".repeat(5_000);
        let truncated = truncate_output_line(&line);

        assert_eq!(truncated.chars().count(), 4_099);
        assert!(truncated.ends_with("..."));
    }

    #[cfg(unix)]
    #[tokio::test]
    async fn permits_only_one_cargo_job_and_stops_it_cleanly() {
        let directory = tempdir().unwrap();
        let executable = directory.path().join("slow-cargo");
        fs::write(&executable, "#!/bin/sh\nsleep 30\n").unwrap();
        let mut permissions = fs::metadata(&executable).unwrap().permissions();
        permissions.set_mode(0o700);
        fs::set_permissions(&executable, permissions).unwrap();
        let project = Project {
            id: ProjectId::new("test").unwrap(),
            name: "Test".to_owned(),
            path: directory.path().to_path_buf(),
            browser_url: None,
        };
        let runner = CargoJobRunner::new(executable);

        let first = runner.start(&project, JobKind::CargoCheck).await.unwrap();
        assert!(matches!(
            runner.start(&project, JobKind::CargoTest).await,
            Err(JobError::Busy)
        ));
        assert!(runner.cancel(first).await);
