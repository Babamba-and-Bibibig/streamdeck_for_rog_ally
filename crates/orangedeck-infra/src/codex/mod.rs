...
mod tests {
    ...
    use orangedeck_domain::{Project, ProjectId};
    ...
    #[tokio::test]
    #[cfg(unix)]
    async fn catalog_reads_later_pages_without_resuming_any_thread() {
...
for line in sys.stdin:
    ...
    if method == 'initialize':
        ...
        assert 'appServer' in request['params']['sourceKinds']
        if request['params']['cursor'] is None:
            result = {'data':[{'id':str(i), 'cwd':'/first', 'preview':'test', 'updatedAt':1000-i} for i in range(100)], 'nextCursor':'page-2'}
        else:
            assert request['params']['cursor'] == 'page-2'
            result = {'data':[{'id':'last', 'cwd':'/second-page-project', 'preview':'test', 'updatedAt':1}], 'nextCursor':None}
    else:
        raise AssertionError('Unexpected method: '+method)
    print(json.dumps({'id':request['id'],'result':result}), flush=True)
").unwrap();
        std::fs::set_permissions(&executable, std::fs::Permissions::from_mode(0o700)).unwrap();
        let client = CodexClient::spawn(&executable, directory.path().join("owned.json"), vec![])
            .await
            .unwrap();
        let threads = client.refresh_threads().await.unwrap();
        assert_eq!(threads.len(), 101);
        assert!(
            threads
                .iter()
                .any(|thread| thread.cwd == "/second-page-project")
        );
        client.shutdown.cancel();
    }

    #[test]
    fn owned_thread_registry_round_trips_without_codex_config_changes() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("owned.json");
        let file = OwnedThreadFile {
            version: 1,
            thread_ids: vec!["thr_b".to_owned(), "thr_a".to_owned()],
        };
        write_secure(&path, &serde_json::to_string(&file).unwrap()).unwrap();
        let loaded = load_owned_threads(&path).unwrap();

        assert_eq!(
            loaded,
            HashSet::from(["thr_a".to_owned(), "thr_b".to_owned()])
        );
    }

    #[test]
    fn thread_start_parameters_match_generated_stable_schema() {
        let project = Project {
            id: ProjectId::new("schema-test").unwrap(),
            name: "Schema Test".to_owned(),
            path: PathBuf::from("/tmp/orangedeck-schema-test"),
            browser_url: None,
        };
        let params = thread_start_params(&project);

        assert_eq!(params["sandbox"], "workspace-write");
        assert_eq!(params["approvalPolicy"], "on-request");
        assert_eq!(params["serviceName"], "orangedeck");
    }

    #[tokio::test]
    #[ignore = "requires an installed and authenticated Codex CLI"]
    async fn installed_codex_read_only_smoke_test() {
        let executable = std::env::var_os("ORANGEDECK_CODEX_BIN")
            .map_or_else(|| PathBuf::from("codex"), PathBuf::from);
        let directory = tempdir().unwrap();
        let project = Project {
            id: ProjectId::new("orangedeck-smoke").unwrap(),
            name: "OrangeDeck Smoke".to_owned(),
            path: std::env::current_dir().unwrap().canonicalize().unwrap(),
            browser_url: None,
        };
        let client = CodexClient::spawn(
            &executable,
            directory.path().join("owned-threads.json"),
            vec![project],
        )
        .await
        .unwrap();
        ...
        let _threads = client.refresh_threads().await.unwrap();
        let _limits = client.refresh_limits().await.unwrap();
