...
<td width="50%">
<b>02 · Shortcuts</b><br>
<a href="docs/screenshots/en-shortcuts.png"><img src="docs/screenshots/en-shortcuts.png" alt="Shortcuts tab with Approve and Reject keys highlighted for a simulated approval request" width="640"></a><br>
A touch-friendly 5 × 2 deck. Approve or reject the displayed request once; assign any of 14 actions to the other eight keys.
</td>
<td width="50%">
<b>03 · Projects</b><br>
<a href="docs/screenshots/en-projects.png"><img src="docs/screenshots/en-projects.png" alt="Projects tab showing demo folders, conversation counts and recent activity" width="640"></a><br>
...
<a href="docs/screenshots/en-conversations.png"><img src="docs/screenshots/en-conversations.png" alt="Conversations tab with a pinned demo conversation and other conversations in the project" width="640"></a><br>
Pin the conversation you care about, or follow the latest activity automatically.
</td>
<td width="50%">
<b>05 · Notifications</b><br>
<a href="docs/screenshots/en-notifications.png"><img src="docs/screenshots/en-notifications.png" alt="Notifications tab with the current demo question, response and explicit command approval details" width="640"></a><br>
...
| **Shortcuts** | Use a 5 × 2 deck with **01 Approve / 02 Reject**, plus eight keys you can assign from 14 actions. A new approval for the selected current turn opens this view once. |
| **Projects** | Browse folders discovered from Codex conversation history and change the shared project selection. |
| **Conversations** | Pin a conversation or follow recent activity within the current project. |
| **Notifications** | See the current question, response, completion and explicit approval details. Touch and controller actions decide the displayed request once. |

- Dark surfaces with coral, violet and mint accents, original vector icons and square touch targets.
- **+ → choose an action** to customize your deck. **Quick setup** fills empty keys; **Edit keys** changes or clears them.
...
1. Click **한국어 / EN** to change the interface language instantly. Your actual conversations and project names stay in their original language.
2. Open **Shortcuts**, tap an empty **+** key and choose a function. Assignment saves it; only pressing the assigned key runs it.
3. Try **Quick setup** to fill empty keys with Refresh, Follow latest, Conversations, Notifications, Previous/Next chat, Open editor and Open terminal. Existing assignments are preserved.
4. Choose **Edit keys → a key** to change it, or **Clear key** in the editor to remove it. Press **Done** when finished. A mouse right-click also opens the editor.

| Available action | Result |
| --- | --- |
| LIVE / Projects / Conversations / Notifications | Jump to a tab |
| Refresh / Follow latest | Read fresh host data / follow activity in this project |
| Previous/Next project / Previous/Next chat | Switch project or pin a conversation |
| Open editor / Open terminal / Open folder | Open the registered project **on your main PC** |
| Open website | Open the project's configured `browser_url` on the host |

**Open editor** uses an installed Zed, VS Code or VSCodium: in macOS system/user Applications, or the Agent's PATH on Linux. **Open terminal** uses Terminal on macOS, or Konsole, GNOME Terminal or Xfce Terminal on Linux. These four host open actions require an Agent-registered project; browsing a folder from Codex history does not register it. See [host shortcut settings](docs/INSTALL.md#host-shortcuts) for websites and additional projects.

![English key editor: choose an action to save it to a custom key](docs/screenshots/en-key-editor.png)

Keys 01/02 remain fixed approval decisions. Inside the editor, **A saves and B cancels**; neither decides a background approval. Language and eight key assignments are stored in **`ui-preferences.toml` beside the UI configuration**, normally `~/.config/orangedeck/ui-preferences.toml`. No manual editing is needed. Pairing tokens and Codex settings are separate. Demo preferences last only for that session.
