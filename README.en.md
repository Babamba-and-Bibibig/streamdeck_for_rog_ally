...
The Mac does the work. The Ally is your remote. You do not need Codex on the Ally.

![A macOS Mac runs Codex and Agent. A ROG Ally running CachyOS Handheld runs the remote. Tailscale connects them. Steam Deck and SteamOS are untested.](docs/diagrams/device-roles-en.svg)

[Install](#install) · [See the app](#see-the-app) · [Setup help](docs/INSTALL.en.md)
...
| --- | --- | --- |
| **Work Mac** — developed around a Mac Studio | **macOS** | Codex CLI + **OrangeDeck Agent**. Agent sends Codex work status to the Ally. |
...
Version **0.1.21** is an early release. The UI has been run on a CachyOS Ally. A fresh Mac installation of the latest version, followed by new alerts, token readings and approvals between the two devices, has not yet been verified.
...
- **Both devices:** use **Code → Download ZIP** at the top of this repository, then extract it. Git and SSH keys are not required.

### 1. Install Agent on the Mac
...
2. When setup finishes, double-click **Start OrangeDeck Agent.command**. **Leave its terminal open.**
...
**Start Agent on the Mac first, then the remote on the Ally.** Close the Ally app window to stop the remote. Press **Ctrl+C** in the Mac Agent terminal to stop Agent. Ordinary restarts do not need installation or another connection-file transfer.
