...
1. 생성된 `start-connector.command` 또는 소스 폴더의 `Start OrangeDeck Connector.command`로 통신 모듈을 실행합니다.
2. **다른 터미널**에서 설치기가 알려준 `enable-notifications.command` 또는 소스 폴더의 `Enable Codex Notifications.command`를 실행합니다.
3. Mac에서 평소 쓰는 Codex의 `/hooks`를 열고 OrangeDeck 연결을 확인한 뒤 신뢰합니다. 현재 대화에 보이지 않으면 작업이 끝난 뒤 같은 대화를 다시 열어 확인합니다.
...
0.1.23는 통신 모듈의 실행 파일 이름을 Connector로 바꿨습니다. 0.1.21 이하에서 업데이트하면 **새 통신 모듈 실행 → 알림 설치 실행기 재실행 → `/hooks` 재검토·신뢰**가 한 번 필요합니다. 직접 CLI를 사용할 때도 통신 모듈과 같은 `--config`를 지정하세요. 생성된 실행기는 이를 자동으로 지정합니다.
