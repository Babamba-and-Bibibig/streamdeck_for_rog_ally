...
OrangeDeck separates state and command policy from operating-system, Codex, network and UI adapters. Agent and UI are the composition roots. Installation and publication are independent tools; the running Rust applications do not depend on Python.
...
```mermaid
flowchart TD
  Agent[Agent: composition and HTTP adapter] --> Application
...
| `orangedeck-agent` | Selects paths and concrete backend at startup; composes authenticated routes, jobs, hooks and state publication. |
...
Permission requests carry bounded complete action details and wait up to 120 seconds for an explicit authenticated decision. An unavailable Agent, timeout or no user decision falls back to Codex's ordinary approval handling. Completion/stop events are deduplicated across hooks and newly appended history records; initial history does not replay old alerts. The Agent retains a bounded notification history in memory, not a durable audit database.
