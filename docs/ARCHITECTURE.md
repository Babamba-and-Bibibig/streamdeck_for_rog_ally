...
The opt-in `PreToolUse`/`PostToolUse` hooks use the existing private socket. They match `Bash`, `apply_patch` and MCP tools and compare eligible regular files in the session cwd before and after the call, including side effects of nonzero shell exits. They do not parse shell commands or send tool output. Already-dirty but unchanged files are omitted. Descriptor-relative opens reject symlinks at every directory level; common credential and generated directories are excluded. Each scan checks a 250 ms deadline and at most 8192 entries, reading at most 8 MiB of text with a 256 KiB per-file limit. These are bounded local observations, not proof of the writer: concurrent edits by a human or an unobserved process cannot be attributed. Overlapping observed conversations, missed hooks and limits mark records incomplete. Four pending captures and sixteen retained turns bound storage. Only bounded turn diffs are atomically saved in the private hook directory at turn end; source snapshots stay in memory. Matching captures are reapplied after history refresh and restart. Tool hooks always return an empty decision and leave approvals unchanged. See the [official Codex hooks](https://learn.chatgpt.com/docs/hooks).
