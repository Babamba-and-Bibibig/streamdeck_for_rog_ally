...
**0.1.35의 파일 본문 수집·보관 축소를 적용하려면 Mac 통신 모듈도 업데이트해야 합니다.** 이전 Mac 0.1.31과 파일 목록·언어 전환은 계속 호환되지만, Ally만 갱신하면 Mac의 기존 수집·캐시 처리는 바뀌지 않습니다. 두 기기 모두 아래 순서로 업데이트하세요.
...
**아래 행의 파일 버튼은 수정 파일 목록을 엽니다. 목록에서 파일을 선택하면 Mac 편집기의 기록된 변경 줄로 이동합니다.** 처음 설치할 때 편집기를 고릅니다. Enter를 누르면 설치된 앱을 Zed → VS Code → Cursor → VSCodium 순서로 찾습니다.
...
**에이전트들 → 그 대화의 아래 버튼**을 누르면 파일명·전체 경로·변경 줄 목록이 나옵니다. 목록을 열거나 새로 고칠 때는 편집기를 실행하지 않으며, **목록에서 파일을 선택하면 Mac에서 엽니다.** **Codex 대화의 작업 폴더를 자동으로 사용하므로 폴더를 따로 등록하거나 설정 파일에 추가할 필요가 없습니다.** 새로 만든 파일과 하위 폴더의 파일도 같은 방식으로 엽니다.

상대 파일 경로는 해당 대화의 작업 폴더를 기준으로 찾습니다. 그 폴더 안에서 해당 질의에 기록된 파일을 열며, 삭제한 파일은 이름·경로·삭제 표시만 보이며 열 수 없습니다. 변경 줄 위치가 기록되지 않았으면 파일 첫 줄부터 엽니다.

파일 열기가 실패하면 목록 위에 이유와 **다시 시도**가 나옵니다. 작업 폴더 자동 연결은 0.1.26부터 지원합니다. 구버전 Mac 통신 모듈이 폴더 등록 오류를 보내면 Ally에 업데이트 안내가 나옵니다.

Mac 파일 시스템 이벤트로 바뀐 경로만 받아 전체 프로젝트를 탐색하지 않습니다. **0.1.35부터 수정 파일 목록을 위해 파일 본문을 읽거나 보관·전송하지 않습니다.** 경로·변경 종류·기록된 줄 번호만 남깁니다. 수집은 이후 작업부터 적용하며, 기록되지 않은 과거 변경을 복원하지는 못합니다. 질문·응답과 승인 판단에 필요한 상세 원문은 계속 표시합니다.
