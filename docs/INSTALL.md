...
**0.1.27에서 파일 기록 수집을 보완했습니다.** 수정한 파일이 계속 ‘파일 수정 없음’으로 나온다면 두 기기를 업데이트하세요. 기록이 부족하거나 아직 읽는 중이면 아래 버튼이 **파일 정보 확인 / 확인 중**으로 표시됩니다. 눌러서 창을 열고 기록을 다시 확인할 수 있습니다. 응답·승인·파일은 **에이전트들**에서 보며, 별도 알림 탭은 없습니다.
