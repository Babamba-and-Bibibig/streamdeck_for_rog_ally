...
상대 파일 경로는 해당 대화의 작업 폴더를 기준으로 찾습니다. 그 폴더 안에서 해당 질의에 기록된 파일을 열며, 삭제한 파일은 변경 내용만 표시합니다. 변경 줄 위치가 기록되지 않았으면 파일 첫 줄부터 엽니다.

파일 열기가 실패하면 목록 위에 이유와 **다시 시도**가 나옵니다. 작업 폴더 자동 연결은 0.1.26부터 지원합니다. 구버전 Mac 통신 모듈이 폴더 등록 오류를 보내면 Ally에 업데이트 안내가 나옵니다.

**0.1.29부터 Mac에서 도구 실행 전후의 파일 변경도 수집합니다.** 파일 편집이나 셸 명령으로 만든 파일·수정 내용이 해당 질의의 목록에 반영됩니다. 업데이트 후 Mac의 **Enable Codex Notifications.command**를 다시 실행하고 Codex **`/hooks`에서 새 훅을 확인·신뢰**하세요. 그 뒤 실행하는 작업부터 수집합니다. 이전에 기록되지 않은 변경 내용을 나중에 복원하는 기능은 아닙니다.

수집 범위는 그 대화의 Mac 작업 폴더입니다. 링크·인증 파일·`.env`·주요 빌드/의존성 폴더는 제외하며, 큰 파일은 이름만 보일 수 있습니다. 다른 대화가 같은 폴더에서 동시에 작업해 구분할 수 없거나 수집 한도에 걸리면 불완전한 기록으로 표시합니다. 기록이 부족하거나 아직 읽는 중이면 아래 버튼이 **파일 정보 확인 / 확인 중**으로 표시됩니다. 눌러서 창을 열고 다시 확인할 수 있습니다. 응답·승인·파일은 **에이전트들**에서 보며, 별도 알림 탭은 없습니다.
