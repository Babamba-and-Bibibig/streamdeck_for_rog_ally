...
**0.1.30은 Mac의 파일 변경 이벤트로 수정·생성된 파일만 가져옵니다.** 프로젝트 전체를 훑지 않으므로, 무관한 파일이 많아 수집이 중단되던 문제를 해결했습니다. Codex가 제공한 변경 내역이 있으면 보여주고, 이전 내용이 없는 셸 편집은 **현재 파일 내용**으로 구분합니다. 업데이트 후 Mac의 **Enable Codex Notifications.command**를 다시 실행하고 Codex **`/hooks`에서 새 훅을 확인·신뢰**하세요. 그 뒤 실행하는 작업부터 수집합니다. 이전에 기록되지 않은 변경 내용을 나중에 복원하는 기능은 아닙니다.

수집 범위는 그 대화의 Mac 작업 폴더입니다. 링크·인증 파일·`.env`·주요 빌드/의존성 폴더는 제외하며, 큰 파일은 미리보기가 잘릴 수 있고, 바이너리 파일은 이름만 보일 수 있습니다. 다른 대화가 같은 폴더에서 동시에 작업해 구분할 수 없거나 운영체제가 이벤트 누락을 알리면 불완전한 기록으로 표시합니다. 한 번에 너무 많은 파일이 바뀐 경우에도 같은 안내가 나옵니다. 기록이 부족하거나 아직 읽는 중이면 아래 버튼이 **파일 정보 확인 / 확인 중**으로 표시됩니다. 눌러서 창을 열고 다시 확인할 수 있습니다. 응답·승인·파일은 **에이전트들**에서 보며, 별도 알림 탭은 없습니다.
