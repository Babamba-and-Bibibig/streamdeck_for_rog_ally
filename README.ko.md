...
| 상황 | 설정은 어떻게 되나요? |
| --- | --- |
| **처음 설치** | 내 Mac의 작업 폴더·편집기를 고릅니다. 설치기가 **내 Mac의 주소를 확인하고 새 연결 인증값을 만듭니다.** 이렇게 만든 연결 파일을 내 Ally에 등록합니다. |
| **같은 기기에 업데이트** | 이 기기에 저장된 설정을 그대로 씁니다. **작업 폴더·편집기·연결 파일을 다시 묻지 않는 것이 정상입니다.** |

개인 설정은 각 기기의 **`~/.config/orangedeck/`**에 따로 저장하며, GitHub 다운로드에는 들어 있지 않습니다. 다른 사람이 앱을 받는 것만으로 내 Mac에 연결되지는 않습니다. 연결하려면 **내 Mac에 접속할 수 있는 Tailscale 연결과 내 연결 인증값**이 모두 필요합니다. [설정을 다시 묻지 않는 이유](docs/INSTALL.md#saved-settings).
...
그 안의 **`orangedeck-pairing.toml`**을 USB 등으로 **내 Ally의 다운로드 폴더**에 옮깁니다. **내 Mac의 주소와 비밀 연결 인증값이 담긴 파일**입니다. **이 파일은 GitHub나 채팅에 올리지 마세요.**
