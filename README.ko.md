...
**Mac 설치 → 연결 파일 옮기기 → 핸드헬드에 리모컨 설치**, 세 단계입니다. 현재 버전은 **0.1.35**입니다.
...
**0.1.35는 Mac 통신 모듈도 업데이트하세요.** 수정 파일 목록용 코드 본문의 보관·전송을 없애고, 읽을 수 있는 기존 파일 변경 캐시에서도 본문을 제거합니다. 파일 선택과 기록된 변경 줄로 이동하는 방식은 같습니다.
...
아래 조건은 OrangeDeck 자체 저작물에 적용됩니다. **외부 라이브러리·글꼴은 원래 저작권자의 라이선스를 따르며, 아래 제한으로 그 권리를 줄이지 않습니다.**

**개인 용도와 회사 내부 업무에 무료로 사용할 수 있습니다.** 소스를 확인하고 수정 없이 설치하세요. 개인 설정 변경과 자신의 작업 파일 편집은 자유롭고, 앱을 이용해 유료 업무를 하는 것도 허용됩니다.

**앱 자체의 수정·재배포·판매는 사전 서면 허락이 필요합니다.** 무료로 다시 올리는 것과 수정판 배포도 해당합니다. 제작자를 사칭하거나 저작권 고지를 지우면 안 됩니다. 공식 다운로드 링크와 앱 소개용 스크린샷은 공유할 수 있습니다.

오류 제보와 기능 제안은 환영합니다. 코드·번역·그림·문서의 수정안을 앱에 반영하는 기여는 별도 서면 합의 후 받습니다. 조건을 위반하면 이 라이선스로 받은 사용 허락이 종료됩니다.

[전체 이용 조건](LICENSE) · [외부 라이브러리·글꼴 고지](THIRD_PARTY_NOTICES.md). 외부 구성요소, GitHub 약관의 열람·포크, 이전 라이선스로 받은 사본의 권리는 유지됩니다.
