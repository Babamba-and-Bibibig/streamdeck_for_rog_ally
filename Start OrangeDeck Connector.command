...
ORANGEDECK_CONFIG="${XDG_CONFIG_HOME:-$HOME/.config}/orangedeck/connector.toml"
...
if [[ "$OSTYPE" != darwin* || ! -f "$ORANGEDECK_ROOT/Cargo.toml" || ! -f "$ORANGEDECK_CONFIG" ]]; then
  ...
  print -u2 "First install / 처음 설치: Setup OrangeDeck.command 또는 sh install.sh --role connector"
...
if [[ ! -x "$ORANGEDECK_CONNECTOR" ]]; then
  print "첫 실행: 이 새 소스 폴더 안에서 Mac 통신 모듈을 빌드합니다."
  print "기존 설치, 페어링, Codex 설정과 대화는 변경하지 않습니다."
  if ! ./scripts/build-macos-connector.zsh; then
    print -u2 "빌드 실패. 기존 Mac 통신 모듈은 그대로 사용할 수 있습니다."
    read -r "?Press Return to close. "
    exit 1
  fi
  "$ORANGEDECK_CONNECTOR" doctor --config "$ORANGEDECK_CONFIG" --codex-monitor || {
...
print "OrangeDeck Mac 통신 모듈 — 이 Terminal을 열어 두세요. 종료: Ctrl-C."
print "이전 통신 모듈이 실행 중이면 그 터미널에서만 Ctrl-C로 종료하세요."
"$ORANGEDECK_CONNECTOR" serve --config "$ORANGEDECK_CONFIG" || {
  orange_exit_code=$?
  print -u2 "Connector stopped with exit code $orange_exit_code."
  print -u2 "주소 사용 중 오류라면 이전 OrangeDeck 통신 모듈만 종료하고 이 파일을 다시 실행하세요."
  read -r "?Press Return to close. "
