...
1. **Setup OrangeDeck.command**를 두 번 누릅니다. 작업 폴더를 물으면 **Mac에서 Codex로 작업할 폴더**를 넣습니다. 기기 이름과 프로젝트 이름은 Enter로 기본값을 써도 됩니다. 편집기는 `zed`, `vs_code`, `cursor`, `vscodium` 중에서 고르거나 Enter로 자동 선택합니다.
...
- **위 버튼:** 질문과 응답을 창으로 보여줍니다. 승인할 일이 있으면 내용을 읽고 **승인 / 거부**을 누르세요. Mac에 전달되면 창이 닫힙니다.
